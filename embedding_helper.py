...
def get_embeddings(data, sentences, text_len, context_word_emb, head_word_emb, char_index, lm_emb,
  ...
  if data.char_embedding_size > 0:
    ...
    flattened_aggregated_char_emb = util.cnn(
        ...
        data.config["filter_size"])# [num_sentences * max_sentence_length,  num-filters * len(filter-sizes)]
  ...
  context_emb = tf.concat(context_emb_list, 2)  # [num_sentences, max_sentence_length, emb concated]
